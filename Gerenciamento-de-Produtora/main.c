...
struct cadastro{
    ...
    int data[3];
    int horario[3];
}cerimonias[5];//Limite de Cerimonias que podem ser cadastradas = 5
...
void main()
{
    ...
    void buscaPorData(int data[]);//Item B
    void buscaPorNomes(char nomeNoivo[], char nomeNoiva[]);//Item C
    ...
    while(opcao != 'S'){
        ...
        //Variaveis de entrada para as funcoes
        int data[3];
        char nomeNoivo[30];
        char nomeNoiva[30];

        //Controle de escolha de opcoes
        ...
        switch(opcao){
            ...
            case 'C':
                ...
                scanf("%s", &nomeNoiva);
                setbuf(stdin, NULL);
                buscaPorNomes(nomeNoivo, nomeNoiva);
                break;
            case 'D':
                listarCadastrados();
                break;
...
{
    if (contadorCerimonias < 5){ //Verifica se o limite de Cadastros foi alcansado
...
void buscaPorData(int data[])
{
    int encontradas = 0;
    if (contadorCerimonias < 1) printf("Nenhuma Cerimonia Cadastrada\n\n");
    else{
        for(int x = 0; x < 5; x++){ //Percorre o vetor de estruturas cerimonias
...
void buscaPorNomes(char nomeNoivo[], char nomeNoiva[])
{
    int encontradas = 0;
    if (contadorCerimonias < 1) printf("Nenhuma Cerimonia Cadastrada\n\n");
    else{
